    ...
    def _calculate_eao(self, tracker_name, tags):
        ...
        for video in self.dataset:
            ...
            for tracker_traj in tracker_trajs:
                ...
                failures = calculate_failures(tracker_traj)[1]  # the tracker fails at failures
                all_overlaps.append(overlaps)
                all_failures.append(failures)
        fragment_num = sum([len(x)+1 for x in all_failures]) # the total number of failures + 60
        max_len = max([len(x) for x in all_overlaps]) # the longest sequence
        ...
        for tag in tags:
            ...
            for name, traj_len, failures, overlaps in zip(video_names, gt_traj_length,
                ...
                if len(failures) > 0:
                    ...
                    points.insert(0, 0) #set the initial point
            ...
            expected_overlaps = calculate_expected_overlap(fragments, fweights)#纵向
            # caculate eao
