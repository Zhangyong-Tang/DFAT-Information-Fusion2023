...
class VOTRGBTVideo(Video):
    ...
    def __init__(self, name1, name, root, video_dir, init_rect, img_names_rgb, img_names_tir, gt_rect,
                 camera_motion, illu_change, motion_change, size_change, occlusion, load_img=False):
        super(VOTRGBTVideo, self).__init__(name, root, video_dir,
                                           init_rect, [img_names_rgb, img_names_tir], gt_rect, None, load_img)
        self.tags= {'all': [1] * len(gt_rect)}
        self.tags['camera_motion'] = camera_motion
        self.tags['illum_change'] = illu_change
        self.tags['motion_change'] = motion_change
        self.tags['size_change'] = size_change
        self.tags['occlusion'] = occlusion
        ...
        # empty tag
        all_tag = [v for k, v in self.tags.items() if len(v) > 0 ]
        a = np.zeros((6, len(all_tag[0])))
        for i in range(6):
            a[i, :len(all_tag[0])] = np.array(all_tag[i])[:len(all_tag[0])]
        self.tags['empty'] = np.all(1 - a, axis=1).astype(np.int32).tolist()
        # self.tags['empty'] = np.all(1 - np.array(list(self.tags.values())),
        #         axis=1).astype(np.int32).tolist()

        self.tag_names = list(self.tags.keys())
        if not load_img:
            img_name_rgb = os.path.join(root, self.img_names_rgb[0].replace('\\', '/'))
            img_name_tir = os.path.join(root, self.img_names_tir[0].replace('\\', '/'))
            img_rgb = np.array(Image.open(img_name_rgb), np.uint8)
            img_tir = np.array(Image.open(img_name_tir), np.uint8)
            self.width = img_rgb.shape[1]
            self.height = img_tir.shape[0]
...
class VOTRGBTDataset(Dataset):
    ...
    """
    def __init__(self, name, dataset_root, load_img=False):
        super(VOTRGBTDataset, self).__init__(name, dataset_root)
        with open(os.path.join(dataset_root, name+'.json'), 'r') as f:
            meta_data = json.load(f)# only for gt?
        ...
        for video in pbar:
            pbar.set_postfix_str(video)
            self.videos[video] = VOTRGBTVideo(video,
                                              meta_data[video]['video_name'],
                                              dataset_root,
                                              meta_data[video]['video_dir'],
                                              meta_data[video]['init_rect'],
                                              meta_data[video]['img_names_rgb'],
                                              meta_data[video]['img_names_tir'],
                                              meta_data[video]['gt_rect'],
                                              meta_data[video]['camera_motion'],
                                              meta_data[video]['illu_change'],
                                              meta_data[video]['motion_change'],
                                              meta_data[video]['size_change'],
                                              meta_data[video]['occlusion'],
                                              load_img=load_img)
        ...
        # self.tags = ['all', 'camera_motion', 'illum_change', 'motion_change',
        #              'size_change', 'occlusion', 'empty']
        self.tags = ['all', 'camera_motion', 'illu_change', 'motion_change',
                     'size_change', 'occlusion', 'empty']