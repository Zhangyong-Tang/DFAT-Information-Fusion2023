...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        if len(img_names) != 2:
            ...
            self.img_names_rgb = img_names[0]
            self.img_names_tir = img_names[1]
        self.imgs = None
        ...
        if load_img:
            ...
            if len(img_names) != 2:
                ...
                img_rgb = cv2.imread(os.path.join(root, self.img_names[0][0].replace('\\', '/')))
                img_tir = cv2.imread(os.path.join(root, self.img_names[1][0].replace('\\', '/')))
                assert (img_rgb is not None) and (img_tir is not None), self.img_names[0][0]
                self.width = img_rgb.shape[1]
                self.height = img_rgb.shape[0]
    ...
    def load_img(self):
        if self.imgs is None:
            if len(self.img_names) != 2:
                ...
                self.img_rgb = [cv2.imread(os.path.join(self.root, self.img_names[0][i].replace('\\', '/'))) for i in range(len(self.img_names[0]))]
                self.img_tir = [cv2.imread(os.path.join(self.root, self.img_names[1][i].replace('\\', '/'))) for i in range(len(self.img_names[1]))]
                # assert (img_rgb is not None) and (img_tir is not None), self.img_names[0][0]
                self.width = self.img_rgb[0].shape[1]
                self.height = self.img_rgb[0].shape[0]
    ...
    def __getitem__(self, idx):
        if self.imgs is None:
            return [cv2.imread(os.path.join(self.root, self.img_names_rgb[idx].replace('\\', '/'))), \
                    cv2.imread(os.path.join(self.root, self.img_names_tir[idx].replace('\\', '/')))], self.gt_traj[idx]
        else:
            return self.imgs[idx], self.gt_traj[idx]

    def __iter__(self):
        for i in range(len(self.img_names[0])):
            if self.imgs is not None:
                yield self.imgs[i], self.gt_traj[i]
            else:
                if len(self.img_names) != 2:
                    yield cv2.imread(self.img_names[i]), self.gt_traj[i]  # start i+1 from i(not 0)
                else:
                    yield [cv2.imread(os.path.join(self.root, self.img_names_rgb[i].replace('\\', '/'))), \
                           cv2.imread(os.path.join(self.root, self.img_names_tir[i].replace('\\', '/')))], self.gt_traj[i]  # start i+1 from i(not 0)
