...
from DFAT.core.xcorr import xcorr_fast, xcorr_depthwise
from DFAT.models.init_weight import init_weights
...
class DepthwiseXCorr(nn.Module):
    ...
    def corrfeature(self, kernel, search):
        if isinstance(kernel, list):
            kernel = self.conv_kernel(torch.cat(kernel, 1))
            search = self.conv_search(torch.cat(search, 1))
        else:
    ...
    def forward(self, kernel, search):
        feature = self.corrfeature(kernel, search)
    ...
    def __init__(self, anchor_num, in_channels, weighted=False):
        ...
        for i in range(len(in_channels)):
            if cfg.ANCHOR.TYPE:
                ...
                self.add_module('rpn' + str(i + 2),
                                DepthwiseRPN(1, in_channels[i], in_channels[i]))
        if self.weighted:
