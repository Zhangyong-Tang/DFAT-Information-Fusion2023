        ...
        for kk in range(100):
            ...
            try:
                ...
                if dataset.has_mask and not neg:
                    ...
                    template_mask = (cv2.imread(template[2], 0) > 0).astype(np.float32)
                else:
                    search_mask = np.zeros(search_image.shape[:2], dtype=np.float32)
                    template_mask = np.zeros(template_image.shape[:2], dtype=np.float32)
                ...
                # augmentation
                template, bbox_, mask_t = self.template_aug(template_image,
                                                       template_box,
                ...
                if cfg.TRAIN.DEBUG:
                    ...
                    draw(template, bbox_, "debug/{:06d}_t.jpg".format(index))
                    draw(search, bbox, "debug/{:06d}_s.jpg".format(index))
                    draw(mask.astype(np.uint8) * 255, bbox, "debug/{:06d}_sm.jpg".format(index))
                ...
                # mask_t = (np.expand_dims(mask_t, axis=0) > 0.5) * 2 - 1
